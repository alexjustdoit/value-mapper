...
def _build_export_text(scenario_name: str, product_name: str, customer, calculator) -> str:
    lines = [
        ...
        f"Product: {product_name}  |  Industry: {customer.industry}  |  Size: {customer.company_size}",
        "",
        "── INPUTS " + "─" * 52,
    ]
    for field in calculator.fields:
        val = field.current_value if field.current_value is not None else field.ai_estimate
        status = "adjusted" if field.current_value is not None else "AI estimate"
        lines.append(f"  {field.label} ({field.unit}): {val}  [{status}]")
    lines += ["", "── ROI SUMMARY " + "─" * 47]
    for metric in calculator.output_metrics:
        value_str = metric.format_value(calculator.fields)
        lines.append(f"  {metric.label}: {value_str}")
        lines.append(f"    {metric.description}")
    lines += ["", "── RATIONALE " + "─" * 49, f"  {calculator.rationale}", "", "Generated by Value Mapper"]
    return "\n".join(lines)
...
def _render_calculator() -> None:
    ...
    col_save, col_dl, col_restart = st.columns([2, 2, 2])

    export_text = _build_export_text(scenario_name, product.name, customer, calculator)
    col_dl.download_button(
        "⬇️  Export",
        data=export_text,
        file_name=f"{scenario_name.replace(' ', '_')}.txt",
        mime="text/plain",
