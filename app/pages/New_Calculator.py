...
from data.store import list_products, load_scenario, save_product, save_scenario
...
_SCENARIO_ID = "nc_scenario_id"    # str — ID of the auto-saved scenario
...
def _regen_dialog() -> None:
    ...
    old_estimates = {f.key: f.ai_estimate for f in calculator.fields}
    adjusted = [f for f in calculator.fields if field_vals.get(f.key, f.ai_estimate) != f.ai_estimate]
            ...
            try:
                ...
                if keep and adjusted:
                    adjusted_vals = {
                        ...
                        if v != old_estimates.get(k, v)
                    }
                    for field in new_calc.fields:
                        if field.key in adjusted_vals:
                            field.current_value = adjusted_vals[field.key]
                st.session_state[_CALCULATOR] = new_calc.model_dump(mode="json")
                if keep and adjusted:
                    ...
                    st.session_state[_FIELD_VALS] = {f.key: f.ai_estimate for f in new_calc.fields}
                # Update the auto-saved scenario on disk
                scenario_id = st.session_state.get(_SCENARIO_ID)
                if scenario_id:
...
@st.dialog("Save Calculator")
def _save_dialog() -> None:
    from data.models import Calculator
    from data.store import load_scenario
    scenario_id = st.session_state.get(_SCENARIO_ID)
    scenario_name = st.session_state.get(_SCENARIO_NAME, "")
    field_vals = st.session_state.get(_FIELD_VALS, {})

    new_name = st.text_input("Scenario name", value=scenario_name)
    st.caption("Leave as-is to keep the auto-saved name.")
    col_confirm, col_cancel = st.columns(2)
    if col_confirm.button("Save", type="primary", use_container_width=True):
        scenario = load_scenario(scenario_id)
        for field in scenario.calculator.fields:
            v = field_vals.get(field.key)
            field.current_value = v if v != field.ai_estimate else None
        scenario.name = new_name.strip() or scenario_name
        save_scenario(scenario)
        _reset()
    ...
    if _CALCULATOR not in st.session_state:
        with st.spinner(f"Building a custom ROI calculator for {customer.company_name}…"):
            try:
                ...
                # Auto-save so the scenario is never lost
                ...
                save_scenario(scenario)
                st.session_state[_SCENARIO_ID] = scenario.id
                st.session_state[_SCENARIO_NAME] = default_name
...
def _render_calculator() -> None:
    ...
    # ── Action bar ────────────────────────────────────────────────────────────
    scenario_name = st.session_state.get(_SCENARIO_NAME, f"{customer.company_name} — {product.name}")
    st.caption(f"✅ Auto-saved as **{scenario_name}**")

    col_save, col_regen, col_dl, col_restart = st.columns([2, 2, 2, 2])

    pdf_bytes = build_export_pdf(scenario_name, product.name, customer, calculator)
    col_dl.download_button(
        "⬇️  Export PDF",
        data=pdf_bytes,
        file_name=f"{scenario_name.replace(' ', '_')}.pdf",
        mime="application/pdf",
        use_container_width=True,
    )

    if col_save.button("💾  Rename & Save", type="primary", use_container_width=True):
        _save_dialog()
